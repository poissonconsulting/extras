...
And secondly, it is also computationally problematic, because calculating deviance residuals, as we will do in the next section, involves taking the square root of the deviance.
...
Thus, we must in general search for the value of $p$ that maximizes the likelihood for each data point in order to calculate the saturated log-likelihood.
...
When $p$ deviates from $0.5$ and $\theta > 0$, the distribution of the deviance residuals becomes increasingly skewed and discontinuous.
...
The reasons behind these differences are linked, and are because the expected value for the saturated model which is used to calculate the sign of the residual ($n \cdot p^*$) becomes increasingly divergent from the actual expected value ($n \cdot p$) with increasing $\theta$. 
