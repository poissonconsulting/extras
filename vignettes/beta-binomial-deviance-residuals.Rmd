...
The problem with the first approach is that likelihood profile of the beta-binomial becomes increasing dispersed relative to the binomial as $\theta$ increases (Fig. 1). 
...
```{r, echo = FALSE, fig.height = 8, fig.width = 7, fig.cap = "Fig. 4: Histograms of deviance residuals for beta-binomial data simulated with $n = 50$, $p = \\{0.3, 0.5, 0.9 \\}$, and $\\theta = \\{0.0, 0.1, 0.5, 1.0 \\}$. The percentages within each panel (i.e., each combination of $p$ and $\\theta$) sum to 100%."}
...
ggplot(x, aes(x = x)) +
  geom_histogram(aes(y = after_stat(count)/sum(after_stat(count))), binwidth = 0.1) +
  scale_y_continuous(labels = percent) +
  ylab("Percent (%)")
```
