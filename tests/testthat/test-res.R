...
test_that("res_norm_skew", {
  expect_identical(res_norm_skew(integer(0), integer(0), integer(0)), numeric(0))
  expect_identical(res_norm_skew(NA, 1, 1, 1), NA_real_)
  expect_identical(res_norm_skew(1, NA, 1, 1), NA_real_)
  expect_identical(res_norm_skew(1, 1, NA, 1), NA_real_)
  expect_identical(res_norm_skew(1, 1, 1, NA), NA_real_)
  expect_equal(res_norm_skew(1, 3, 0.5), dev_norm_skew(1, 3, 0.5, res = TRUE))
  expect_equal(res_norm_skew(1, 3, 0.5, 1.0), dev_norm_skew(1, 3, 0.5, 1.0, res = TRUE))
  expect_equal(res_norm_skew(0, 1, 0.5), -2)
  expect_equal(res_norm_skew(1, 1, 0.5), 0)
  expect_equal(res_norm_skew(0, 1, 0.7), -1.42857142857143)
  expect_equal(res_norm_skew(1, 1, 0.7), 0)
  expect_equal(res_norm_skew(0, 2, 0.5, 0.5), -4.72416743525476)
  expect_equal(res_norm_skew(1, 2, 0.5, 1), -3.2524653138693)
  expect_equal(res_norm_skew(0, 2, 0.7, -4), -2.80920319431421)
  expect_equal(res_norm_skew(1, 2, 0.7, -2), -1.20493839504062)
  expect_equal(res_norm_skew(1, 2, 0.5), -2)
  expect_equal(res_norm_skew(5, 10, 0.5), -10)
  expect_equal(res_norm_skew(1, 2, 0.5, -2), -1.84576386463206)
  expect_equal(res_norm_skew(5, 10, 5, 2), -2.82367660468737)
  expect_equal(res_norm_skew(1, 10, 10), res_norm(1, 10, 10))
  expect_equal(res_norm_skew(1, 10, 0.5, 0.5), -20.2486465600944)
  expect_equal(res_norm_skew(1:9, 10, 0.5, 10),
               c(-180.931340047915, -160.835052963763, -140.739643395107, -120.645509483193,
                 -100.553340951113, -80.4644530321068, -60.3817280342612, -40.3130604527845,
                 -20.2918769408978))
  expect_equal(res_norm_skew(1:9, 10, 0.5, -10),
               c(-17.9979431892188, -15.9976860527498, -13.9973554303071, -11.9969145634345,
                 -9.99629726660561, -7.99537110097754, -5.99382674443854, -3.99073414829236,
                 -1.98140330128592))
  expect_equal(res_norm_skew(0, 2, 0.5, type = "raw"), -2)
  expect_equal(res_norm_skew(0, 2, 0.5, 10, type = "raw"), -1.60303759425339)
  set.seed(101)
  expect_equal(res_norm_skew(1:2, simulate = TRUE, type = "raw"), c(0.552461855419139, 0.214359459043425))
  expect_equal(res_norm_skew(1:2, 2, simulate = TRUE), c(1.1739662875627, -0.112734314754215))
  set.seed(101)
  expect_equal(res_norm_skew(1:2, shape = 2, simulate = TRUE, type = "raw"), c(1.25233400157483, 1.41320223730144))
  expect_equal(res_norm_skew(1:2, 2, shape = 2, simulate = TRUE), c(0.403578184606959, -0.0430501884150838))
  set.seed(101)
  expect_equal(res_norm_skew(1:2, simulate = TRUE, type = "raw"), c(0.552461855419139, 0.214359459043425))
  expect_equal(res_norm_skew(1:2, 2, type = "data"), 1:2)
  set.seed(101)
  expect_equal(res_norm_skew(1:2, shape = 2, simulate = TRUE, type = "raw"), c(1.25233400157483, 1.41320223730144))
  expect_equal(res_norm_skew(1:2, 2, shape = 2, type = "data"), 1:2)
  set.seed(101)
  expect_equal(res_norm_skew(1:2, 2, simulate = TRUE, type = "data"), c(2.55246185541914, 2.21435945904343))
  expect_equal(res_norm_skew(1:2, 2, shape = 2, simulate = TRUE, type = "data"), c(2.8029741225477, 2.50304615414932))
  set.seed(101)
  expect_equal(res_norm_skew(1:2, 2, sd = 0.5, shape = 2, simulate = TRUE, type = "standardized"),
               c(-0.713119218167245, -0.0574564315318186))
  expect_equal(res_norm_skew(NA, 6, 0.5, 0.2, type = "standardized"), NA_real_)
  expect_equal(res_norm_skew(5, NA, 0.5, 0.2, type = "standardized"), NA_real_)
  expect_equal(res_norm_skew(5, 6, NA, 0.2, type = "standardized"), NA_real_)
  expect_equal(res_norm_skew(5, 6, 0.5, NA, type = "standardized"), NA_real_)
  expect_equal(res_norm_skew(NA, NA, NA, NA, type = "standardized"), NA_real_)
  expect_equal(res_norm_skew(numeric(0), numeric(0), numeric(0), numeric(0), type = "standardized"), numeric(0))
  set.seed(101)
  expect_equal(res_norm_skew(1:2, 2, sd = 0.5, shape = 0, simulate = TRUE, type = "standardized"),
               c(1.10492371083828, 0.428718918086851))
  set.seed(101)
  res <- res_norm_skew(rep(2, 10000), mean = 10, sd = 0.3, shape = 0.2, simulate = TRUE, type = "standardized")
