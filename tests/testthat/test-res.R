...
test_that("res_skewnorm", {
  ...
  expect_equal(res_skewnorm(1:9, 10, 0.5, 10),
  ...
  set.seed(101)
  expect_equal(res_skewnorm(1:2, 2, sd = 0.5, shape = 2, simulate = TRUE, type = "standardized"),
  ...
  set.seed(101)
  expect_equal(res_skewnorm(1:2, 2, sd = 0.5, shape = 0, simulate = TRUE, type = "standardized"),
...
test_that("res_upois", {
  expect_identical(res_upois(integer(0), integer(0), integer(0)), numeric(0))
  expect_equal(res_upois(1, 1, 1), -0.651640894278772)
  expect_identical(res_upois(1, 1, 1, type = "raw"), -0.5)
  expect_equal(res_upois(1, 1, 0.5, type = "raw"), -1/3)
  expect_equal(res_upois(1, 1, 0.5), -0.405015814635496)
  expect_equal(res_upois(2, 2, 2, type = "raw"), -2/3)
  expect_equal(res_upois(0, 1, 0, type = "raw"), -1)
  expect_identical(res_upois(NA, 1, 1), NA_real_)
  expect_identical(res_upois(1, NA, 1), NA_real_)
  expect_identical(res_upois(1, 1, NA), NA_real_)
  expect_error(res_upois(1, 3, 1, type = "unknown"))
  expect_equal(res_upois(1, 3, 1, type = "raw"), -2.5)
  expect_equal(res_upois(c(1,3.5,4), 3, 10, type = "raw"),
               c(-2.90909090909091, -0.409090909090909, 0.0909090909090908))
  expect_equal(res_upois(c(1,3.5,4), 3, 5, type = "raw"),
               c(-2.83333333333333, -0.333333333333333, 0.166666666666667))
  set.seed(101)
  expect_equal(res_upois(1:2, 2, 2, simulate = TRUE, type = "raw"), c(-0.666666666666667, -2.66666666666667))
  expect_equal(res_upois(1:2, 2, 2, simulate = TRUE), c(-0.535571699965782, -0.535571699965782))
  set.seed(101)
  expect_equal(res_upois(1:2, 2, 0.5, simulate = TRUE, type = "raw"), c(-0.333333333333333, -2.33333333333333))
  expect_equal(res_upois(1:2, 2, 0.5, simulate = TRUE), c(-1.06882485121411, -1.06882485121411))
  set.seed(101)
  expect_equal(res_upois(1:2, 2, 1, simulate = TRUE, type = "data"), c(2L, 0L))
  set.seed(101)
  expect_equal(res_upois(1:2, 2, 1, simulate = TRUE, type = "standardized"), c(-0.333333333333333, -1.66666666666667))
  set.seed(101)
  res <- res_upois(rep(2, 10000), 2, 1, simulate = TRUE, type = "standardized")
  expect_equal(mean(res), 0.00306666666666664)
  expect_equal(sd(res), 1.00071178233097)
})
