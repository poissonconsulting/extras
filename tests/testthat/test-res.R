...
test_that("res_lnorm_hurdle", { ### Copied from lnorm; needs editing for hurdle.
  expect_identical(res_lnorm_hurdle(integer(0), integer(0), integer(0)), numeric(0))
  expect_identical(res_lnorm_hurdle(exp(0)), 0)
  expect_identical(res_lnorm_hurdle(1), 0)
  expect_identical(res_lnorm_hurdle(0), -Inf)
  expect_identical(res_lnorm_hurdle(-1), -Inf)
  expect_identical(res_lnorm_hurdle(NA, 1, 1), NA_real_)
  expect_identical(res_lnorm_hurdle(1, NA, 1), NA_real_)
  expect_identical(res_lnorm_hurdle(1, 1, NA), NA_real_)
  expect_equal(res_lnorm_hurdle(exp(-2:2)), -2:2)
  expect_equal(res_lnorm_hurdle(exp(-2:2), meanlog = -1:3, sdlog = 1:5),
               c(-1, -0.5, -0.333333333333333, -0.25, -0.2))
  expect_equal(res_lnorm_hurdle(1, type = "raw"), 0)
  expect_equal(res_lnorm_hurdle(exp(1), type = "raw"), 1.71828182845905)
  set.seed(101)
  expect_equal(res_lnorm_hurdle(1:2, 2, simulate = TRUE, type = "raw"), c(-2.05579169361621, 5.44961576366372))
  expect_equal(res_lnorm_hurdle(1:2, 2, simulate = TRUE), c(-0.67494384395583, 0.214359459043425))
  set.seed(101)
  expect_equal(res_lnorm_hurdle(1:2, 2, simulate = TRUE, type = "raw"), c(-2.05579169361621, 5.44961576366372))
  expect_equal(res_lnorm_hurdle(1:2, 2, type = "data"), 1:2)
  set.seed(101)
  expect_equal(res_lnorm_hurdle(1:2, 2, simulate = TRUE, type = "data"), c(5.33326440531444, 12.8386718625944))
  set.seed(101)
  expect_equal(res_lnorm_hurdle(1:2, 2, simulate = TRUE, type = "standardized"), c(-0.428902244197459, 0.0410901945157245))
  set.seed(101)
  res <- res_lnorm_hurdle(rep(2, 10000), simulate = TRUE, type = "standardized")
  expect_equal(mean(res), -0.000972725402704599)
  expect_equal(sd(res), 0.96810060574941)
})
