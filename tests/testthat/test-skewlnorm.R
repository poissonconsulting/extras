...
test_that("dskewlnorm errors with character arguments", {
  ...
  expect_chk_error(
    dskewlnorm(character(0), 1:5, 1:5, 1:5),
...
test_that("dskewlnorm equal to dlnorm when shape = 0", {
  ...
  expect_equal(
    dskewlnorm(1:5, 0.3, 0.7, 0, log = TRUE),
    stats::dlnorm(1:5, 0.3, 0.7, log = TRUE)
  )
})
...
test_that("qskewlnorm equal to qlnorm when shape = 0", {
  ...
  expect_equal(
    ...
    stats::qlnorm(c(0.1, 0.5, 0.9), 0.3, 0.7)
  )
})
...
test_that("qskewlnorm is the inverse of pskewlnorm", {
  skip_if_not_installed("sn")
  expect_equal(
    pskewlnorm(qskewlnorm(0.7, 0.3, 0.7, 2), 0.3, 0.7, 2),
    0.7,
    tolerance = 1e-6
  )
})
