...
test_that("p_2_info() returns the correct values", {
  expect_identical(p_2_info(NA_real_), NA_real_)
  expect_identical(p_2_info(NA_integer_), NA_real_)
  expect_identical(p_2_info(0), -Inf)
  expect_identical(p_2_info(.Machine$double.eps), log2(.Machine$double.eps) - log2(1 - .Machine$double.eps))
  expect_identical(p_2_info(1 - .Machine$double.eps), log2(1 - .Machine$double.eps) - log2(.Machine$double.eps))
  expect_identical(p_2_info(1), Inf)

  expect_identical(p_2_info(NA_real_, 1L), NA_real_)
  expect_identical(p_2_info(0, 100), -100)
  expect_identical(p_2_info(.Machine$double.eps, 30), -30)
  p <- runif(1e4)
  expect_equal(p_2_info(p), log2(p) - log2(1 - p))
  expect_identical(p_2_info(1 - .Machine$double.eps, 30), 30)
  expect_identical(p_2_info(1, 1L), 1)
  expect_identical(p_2_info(1, 2), 2)
})
