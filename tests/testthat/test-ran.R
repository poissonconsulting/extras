...
test_that("ran_upois", {
  expect_error(ran_upois(NA_integer_))
  expect_error(ran_upois(NULL))
  expect_error(ran_upois(integer(0)))
  expect_error(ran_upois(-1))
  expect_error(ran_upois(1, -1, 1))
  expect_error(ran_upois(1, 1, -1))
  expect_identical(ran_upois(0L), integer(0))
  expect_warning(ran_upois(1L, 1:3, 1:2), c("longer argument not a multiple of length of shorter"))
  set.seed(101)
  expect_identical(ran_upois(1, 1, 0), 0L)
  expect_identical(ran_upois(1, 2, 0), 1L)
  expect_identical(ran_upois(1, 0, 0), 0L)
  expect_identical(ran_upois(10, 1, 0), c(0L, 1L, 2L, 0L, 1L, 1L, 1L, 3L, 3L, 0L))
  expect_identical(length(ran_upois(10, 1, 0)), 10L)
  expect_identical(ran_upois(1, 1000, 0), 1056L)
  # Vectorized
  set.seed(101)
  expect_identical(ran_upois(2, c(1, 2), 0), c(1L, 1L))
  expect_identical(ran_upois(2, c(1, 1000), 0), c(2L, 1020L))
  expect_identical(ran_upois(10, 10, seq(0, 9, 1)), c(9L, 13L, 10L, 12L, 8L, 16L, 6L, 10L, 10L, 8L))
  # Correct mean
  set.seed(101)
  x <- ran_upois(10000, 10, 0)
  expect_equal(mean(x), 10, tolerance = 0.01)
  set.seed(101)
  x <- ran_upois(10000, 10, 1)
  expect_equal(mean(x), 10 + (1 / (1 + 1)), tolerance = 0.01)
  # Correct variance
  set.seed(101)
  x <- ran_upois(10000, 10, 0)
  expect_equal(var(x), 10, tolerance = 0.01)
  set.seed(101)
  x <- ran_upois(10000, 10, 1)
  expect_equal(var(x), 10 + (1 / (1 + 1)^2), tolerance = 0.01)
})
