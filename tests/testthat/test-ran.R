...
test_that("ran_norm_skew", {
  expect_error(ran_norm_skew(NA_integer_))
  expect_error(ran_norm_skew(NULL))
  expect_error(ran_norm_skew(integer(0)))
  expect_error(ran_norm_skew(1, 0, sd = -1))
  expect_identical(ran_norm_skew(0L), numeric(0))
  set.seed(101)
  expect_equal(ran_norm_skew(), 0.552461855419139)
  expect_equal(ran_norm_skew(2), c(0.214359459043425, 1.1739662875627))
  expect_equal(ran_norm_skew(2, 10), c(9.88726568524578, 9.77674063537274))
  expect_equal(ran_norm_skew(2, c(5,10)), c(4.20515556458494, 8.53318030583653))
  expect_equal(ran_norm_skew(1, c(5,10)), c(4.8066620350025))
  set.seed(101)
  expect_equal(ran_norm_skew(sd = 1), 0.552461855419139)
  expect_equal(ran_norm_skew(2, sd = 2),c(0.428718918086851, 2.3479325751254))
  expect_equal(ran_norm_skew(2, 10, sd = 3), c(9.66179705573736, 9.33022190611821))
  expect_equal(ran_norm_skew(2, c(5,10), sd = 2), c(3.41031112916989, 7.06636061167307))
  expect_equal(ran_norm_skew(1, c(5,10), sd = 1), c(4.8066620350025))
  expect_equal(ran_norm_skew(1, 0, sd = 3), 0.175396493548505)
  expect_equal(ran_norm_skew(1, 0, sd = 4), -8.20123126255853)
  set.seed(101)
  expect_equal(ran_norm_skew(shape = 10), 0.379390442659801)
  expect_equal(ran_norm_skew(2, shape = 10), c(0.692923789421489, 0.426040940417248))
  expect_equal(ran_norm_skew(2, 10, shape = 10), c(10.6045014356932, 10.8902621154531))
  expect_equal(ran_norm_skew(2, 10, shape = 10), c(10.4447454597277, 11.2747158507856))
  expect_equal(ran_norm_skew(2, c(10,100), shape = -1), c(9.69592899188521, 99.4404740107593))
  expect_equal(ran_norm_skew(2, c(10,100), shape = -1), c(7.97203318662633, 100.385208042625))
  expect_equal(ran_norm_skew(1, c(10,100), shape = -1), c(8.77536013503756))
  set.seed(101)
  expect_equal(ran_norm_skew(sd = 1, shape = 5), 0.428051766065545)
  expect_equal(ran_norm_skew(2, sd = 1, shape = 5), c(0.703876239197544, 0.534968020196629))
  expect_equal(ran_norm_skew(2, 10, sd = 3, shape = 5), c(11.7539930699001, 12.5663063681019))
  expect_equal(ran_norm_skew(2, 10, sd = 2, shape = -2), c(8.3473305357842, 6.13398981871521))
  expect_equal(ran_norm_skew(2, c(10,100), sd = 5, shape = -2), c(8.50920292023333, 96.3305141003515))
  expect_equal(ran_norm_skew(2, c(10,100), sd = 0, shape = -1), c(10, 100))
  expect_equal(ran_norm_skew(1, c(10,100), sd = 10, shape = -1), c(-2.24639864962439))
})
