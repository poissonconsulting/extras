...
test_that("log2<- vectorized numeric values set x as expected", {
  ...
  expect_equal(x, c(
    0.03125, 0.0441941738241592, 0.0625, 0.0883883476483184, 0.125,
...
test_that("log2<- matrix of numeric values set x as expected", {
  ...
  expect_equal(x, structure(c(
    0.5, 0.537845293110091, 0.57855511864136, 0.622346294732012,
    ...
    1.60682245313376, 1.72844378656321, 1.85927071001681, 2
  ), dim = 4:5))
})
...
test_that("log10<- vectorized numeric values set x as expected", {
  ...
  expect_equal(x, c(
    1e-05, 3.16227766016838e-05, 1e-04, 0.000316227766016838, 0.001,
...
test_that("log10<- matrix of numeric values set x as expected", {
  ...
  expect_equal(x, structure(c(
    0.1, 0.127427498570313, 0.162377673918872, 0.206913808111479,
    ...
    4.83293023857175, 6.15848211066026, 7.84759970351461, 10
  ), dim = 4:5))
})
