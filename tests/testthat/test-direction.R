test_that("`direction()` returns the correct direction.", {
  ...
  expect_equal(
    direction(c(100, 0.01, 0.01), estimate = mean, threshold = 1),
    "right"
  )
  expect_equal(
    direction(
...
test_that("`direction()` accepts custom, unnamed functions.", {
  expect_no_error(direction(c(100, 0.01, 0.01), estimate = function(.x) {
    exp(mean(log(.x)))
  }))
  expect_error(
    direction(c(100, 0.01, 0.01), estimate = function(.x) as.character(.x[1])),
