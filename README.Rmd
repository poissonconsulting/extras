...
It includes functions to numericise objects; 
simple methods for S3 generics in the universals package;
and R translations of common JAGS and TMB functions.
...
The `numericise()` (or `numericize()`) function converts simple objects to a list of numeric atomic object suitable for input to an analytic engine.
```{r}
library(extras)

data <- data.frame(
  logical = TRUE, real = 1.1, integer = 3L,
...
The `extras` package provides simple methods for S3 generics in the universals package. For example the `dims()` and `ndims()` functions are defined for vectors.
...
`extras` also provides R versions of common JAGS and TMB (C++) functions such as `logit<-` and `pow()`.
